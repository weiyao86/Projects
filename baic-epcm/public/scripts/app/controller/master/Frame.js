Ext.define('EPCM.controller.master.Frame', {
  ...
  logoutSys: function () {
    ...
    Ext.Msg.confirm('提示', '您确认退出当前系统?', function (btn) {
      if (btn === 'yes') {
        Ext.util.ajax({
          url: EPCM.globalConfig.path + '/logout',
          disableCaching: true,
          success: function () {
            window.location.href = EPCM.globalConfig.path;
          }
        })
      }
    });
  ...
  showChangePassword: function () {
    var me = this,
      changePwdWindow = Ext.create('EPCM.view.account.changePassword.Form');
  ...
  loadController: function (pageConfig, tab) {
    var me = this,
      ...
      controllerClassName = EPCM.app.getModuleClassName(controllerName, "controller");
  ...
  createViewport: function (viewportName, controllerName) {
    var me = this,
      ...
      viewportClassName = EPCM.app.getModuleClassName(viewportName, "view");
  ...
  destroyController: function (controllerId) {
    ...
    EPCM.app.eventbus.unlisten(controllerId);
    EPCM.app.controllers.remove({
      id: controllerId
    });
  },
  ...
  getTabPage: function (id) {
    ...
    return EPCM.extjsConfig.pages[id];
  },

  getController: function (controllerName) {
    var me = this;

    return EPCM.app.getController(controllerName);
  },
