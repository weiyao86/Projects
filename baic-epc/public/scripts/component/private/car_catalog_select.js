	...
	function setup(ajax, Mustache) {
		var carCatalogSelect = {
			...
			initBrandSelect: function() {
				...
				self.getRenderData(self.opts.brandSeriesUrl);
			},

			getRenderData: function(urlConfig) {
				var self = this;

				ajax.invoke({
					url: '/online/brandSeries',
					type: 'GET',
			...
			renderTemplate: function(data, $dom) {
				...
				for(var i=0; i<data.length; i++) {
					brand.push({code:data[i]['code'], name: data[i]['name']})
				}

				html = Mustache.render(template, {data: brand});
			...
			getSeries: function($this) {
				...
				for(var i=0; i<data.length; i++) {
					if(data[i].code === val) {
						self.renderTemplate(data[i].series, self.seriesSelect);
					}
				}
			},
			...
			resetSelect: function(index) {
				...
				for(var i = index; i < array.length; i++) {
					array[i].addClass('disabled').find('.form-select-option-list').empty();
